...
# MAGIC %md ### <--- SOLUTION --->
# MAGIC __SOLUTION__
# MAGIC > __a)__ First order conditions say that the loss function has a local minimum at any point where all the partial derivatives are zero. Second order conditions say that when the second degree parital derivatives are postive the loss function is convex up. If we can prove that a loss function's second derivativs are always positive then we can guarantee that any local minimum is the global minimum. Since the goal of gradient descent is to find the global minimum this makes life easier because we don't have to worry about local optima.
# MAGIC 
# MAGIC > __b)__ Problem domain space allows us to visualize a particular model alonside the data - this allows us to visually asses goodness of fit by how close the points are to the model line. In model parameter space goodness of fit is represened in a third dimension as a function of the parameters - this obscured the role of the data but gives us a more precise view of how different models compare in performance. In practice though, we don't actually have access to the full error surface so we can't 'just look at it'. We _do_ have an equation for it, but to visualize the whole surface we'd have to evaluate that equation over a grid of points which is computationally prohibitive for more complicated loss functions (expecially with large datasets since loss is a function of the data).
# MAGIC 
# MAGIC > __c)__ The learning rate controls the size of the updates to our parameters-- we can think of this as the length of the steps in model parameter space, although that length is also a function of the parameters themselves. Large learning rates run the risk of overshooting the true minimum & oscilatting unproductively. However, learning rates that are too small will cause our algorithm to take a long time to converge.
# MAGIC 
# MAGIC > __d)__ The closed form solution comes from solving the system of equations created by the first order conditions (i.e. setting all the paritial derivatives equal to zero and solving for the parameters). In practice this involves inverting a large matrix which is extrememly computationally costly at best, and in the case of more complicated loss functions can actually be impossible.
...
# MAGIC __SOLUTION__
# MAGIC 
# MAGIC > __a)__ Citric acid, dnesity, and ph are all fairly symmetric and could likely be approximated by normality. The others are largely skewed right to varying degrees. Color is a flag variable either 0 or 1 representing whether the wine is red or not. We can interpret this coefficient as the difference in score going from white to red wine holding the other variables in the model constant.
# MAGIC 
# MAGIC > __b)__ Volatile acidity appears to have a negative relationship with quality. Free sulfur dioxide has a positive relationship with quality and so does citric acid. Alcohol has a negative relationship for low levels of alocohol and a positive relationship for high levels of alcohol. For others the relationships are not as clear. No features have a strong and clear positive linear relationship with quality.
# MAGIC 
# MAGIC > __c)__ The most strongly positively correlated features are volatile acidity-color and free sulfur dioxide-total sulfur dioxide. The most strongly negatively correlated features are density-alcohol and total sulfur dioxide-color. We would want to remove some combination of these features in feature selection so they do not interfere with each other's preditive power. One combination to remove could be color and free sulfur dioxide, since they both correlate strongly with total sulfur dioxide.
...
# MAGIC __SOLUTION__
# MAGIC 
# MAGIC > __b)__ OLS loss is the mean square error (MSE) i.e. the average of the residuals. Graphically we can interpret this as the average distance that a point lies from the model hyperplane. In vector notation it is computed using the formula: \\(\frac{1}{N}\sum\_{i=1}^{N} (\theta\cdot x\_i - y\_i)^2\\), where \\(\theta\\) denotes the model parameters and \\(x_i\\) denotes the i-th data point.  
# MAGIC 
# MAGIC > __c)__  The idea of 'augmenting' a data point means to add a vector entry of 1 to correspond with the bias term so that we can apply the model to the data point using vector multiplication without the added step of adding the bias. We do this for convenience of notation & code, it is mathematically equivalent to multiplying the data point by the weights and then adding the bias.  
# MAGIC 
# MAGIC > __d)__ Yes, computing the loss is embarassingly parallel. It can be achieved all in parallel (applying the model, subtracting the true y-value, square-ing the result) except for a single reduce at the end (add & divide to take the mean). 
# MAGIC 
# MAGIC > __e)__ The OLS loss with the baseline model is the same as the variance of the outcome variable. By setting the coefficient of the "bias" to be the mean we are basically getting a model regressing the outcome on constant term only. This makes sense because we are saying each residual is \\(r\_i=y\_i - \bar{y}\\). The mean of this value squared is the definition of population variance.
...
# MAGIC __SOLUTIONS__
# MAGIC > __a)__ $$
# MAGIC g(\hat{\beta}) = \frac{\delta J}{\delta \beta} = \frac{2}{N} \sum_{i=1}^{N} -x_i(y_i - \hat{\beta}x_i)
# MAGIC $$
# MAGIC The \\(x_i\\) is the data and \\((y_i - \hat{\beta}x_i)\\) is the error we are weighting by.
# MAGIC 
# MAGIC > __b)__ Calculating this gradient (each element of the vector) for each training example is fully parallelizable. Once \\(\hat{\beta}\\) is broadcast to the workers, each worker can process each training example \\(i\\) independently. Then these can all be reduced (summed) together to get the full gradient. After the reducing is compelte, then we would make the step in the opposite direction of the gradient to change the \\(\hat{\beta}\\) before re-broadcasting it.
# MAGIC 
# MAGIC > __c)__ _complete the coding portions of this question before answering d & e_ 
# MAGIC 
# MAGIC > __d)__ The loss and coefficients are both exploding as the iterations go on. This is because the features are at different scales. 
# MAGIC 
# MAGIC > __e)__ The data for the run in part d was not normalized, meaning that the scale of the features was extremely different. This makes our error surface in parameter space be contorted into elliptical type shapes instead of spherical, and makes an error in a feature with a larger scale appear more important in the gradient descent algorithm. This can result in taking non-optimal steps. Additionally and more importantly, our learning rate is constant across all features and since some of our features had extremely small scales, this can result in taking steps that are too large and overshooting the optimum for those features.
...
# MAGIC %md ### <--- SOLUTION --->
# MAGIC __SOLUTIONS__
# MAGIC > __a)__ This is the training loss. It is not a test evaluation of the model.
# MAGIC 
# MAGIC > __c)__ Test and train both approach a minimum error with test, as expected, generally having higher loss due to generalization. The gap between the curves gets larger as we train more, likely because we are overfitting.
# MAGIC 
# MAGIC > __d)__ Convergence happens at the same rate but we can see that the relative loss is related to the data we use to train and test. Repeated enough times we can produce a confidence interval for the estimated parameters, but also if we used more training data we would see less variance. Or we could use cross-validation.
...
# MAGIC __SOLUTION__
# MAGIC 
# MAGIC > __a)__ Using hold-one-out CV, we would need to look at \\(\frac{k-1}{k}\\) of the data for each GD iteration-fold. (e.g. 5 folds and 2 iterations would require (4/5)X2X5 = 8 passes over the data).
# MAGIC 
# MAGIC > __b)__ kResiduals emits one tuple for each model-record in the original dataset. The keys identify the model number and whether it is the test set \\(\frac{k-1}{k}\\) of the data or the train set \\(\frac{1}{k}\\) of the data. This will help us calculate the CV loss because we can get the mean of all the test or train keys' values in one pass over the data.
# MAGIC 
# MAGIC > __e)__ The error curves converge to about the same loss that is between the high and low of Q5.
...
# MAGIC __SOLUTION__
# MAGIC 
# MAGIC > __a)__ The gradients still include the same gradient from the unregularized portion above. We just need to add the part that is specific for that element of the parameter vector based on the regularization.
# MAGIC 
# MAGIC $$
# MAGIC J\_{L2} = MSE + \lambda \sum\_{j=1}^{m} \beta\_j^2
# MAGIC $$  
# MAGIC 
# MAGIC $$
# MAGIC \frac{\delta J\_{L2}}{\delta \beta\_j} =  \bigg[\frac{2}{N} \sum\_{i=1}^{N} -x_i(y_i - \hat{\beta}x\_i) \bigg] + 2 \lambda \beta\_j
# MAGIC $$
# MAGIC 
# MAGIC $$
# MAGIC \text{where } N \text{ is the number of observations, and } m \text{ is the number of parameters.}
# MAGIC $$
# MAGIC > __b)__ The gradients still include the same gradient from the unregularized portion above. We just need to add the sign of the coefficient time the regularization parameter to each coefficient's gradient.
# MAGIC 
# MAGIC $$
# MAGIC J\_{L1} = MSE + \lambda \sum\_{j=1}^{m} |\beta\_j| 
# MAGIC $$
# MAGIC 
# MAGIC $$
# MAGIC \frac{\delta J\_{L1}}{\delta \beta\_j} = \bigg[\frac{2}{N} \sum\_{i=1}^{N} -x_i(y\_i - \hat{\beta}x\_i)\bigg] + \lambda sign(\beta\_j)
# MAGIC $$
# MAGIC 
# MAGIC $$
# MAGIC \text{where } N \text{ is the number of observations, and } m \text{ is the number of parameters.}
# MAGIC $$
# MAGIC 
# MAGIC The update is different because it is simpler and does not contain the square, just the sign.
# MAGIC 
# MAGIC > __d)__ Regularization, in both cases, converges the model to approximately the same error as we found in cross-validation. It removed the potential “unluckyness” of the random split. The model seems to generalize well because there is less difference between the training and test sets. Also, it seems that ridge regression converges more smoothly than lasso, presumably because of the squared term (less choppy).
...
def GDUpdate_wReg(dataRDD, W, learningRate = 0.1, regType = None, regParam = 0.1):
    ...
    if regType == 'ridge':
        grad += regParam * 2 * np.append([0.0], W[1:])
    elif regType == 'lasso':
        wReg = W * 1
        wReg[-1] = 0
...
# MAGIC __SOLUTION__
# MAGIC 
# MAGIC > __b)__ Both regularization approaches have a mean squared error higher than OLS. This is contrary to what we would expect. This could be because the held out set is very similar to the training set.  
# MAGIC 
# MAGIC > __c)__ All of our models overestimate the score for low ranked wines and underestimate the score for high ranked wines 
# MAGIC 
# MAGIC > __d)__ Unregularized, the coefficients continue to change up through 50 iterations. The regularized model stabilizes more quickly and appears to stabilize to smaller values (same relative order). Lasso pushes a number of the coefficients to zero. Alcohol and volatile acidity are the largest in magnitude in all three.
...
# get training mean & stdev to replicate normalization
featureMeans = trainRDDCached.map(lambda x: x[0]).mean()
featureStdev = np.sqrt(trainRDDCached.map(lambda x: x[0]).variance())

# prep data
validationRDD = heldOutRDD.map(parse)\
                          .map(lambda x: ((x[0] - featureMeans)/featureStdev, x[1]))\
                          .cache()
